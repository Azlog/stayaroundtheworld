...
$stmt = $pdo->prepare('SELECT SUM(ti.item_price * ti.item_quantity) AS earnings FROM transactions t JOIN transactions_items ti ON ti.txn_id = t.txn_id WHERE cast(t.created as DATE) = cast(now() as DATE) ORDER BY t.created DESC');
$stmt->execute();
